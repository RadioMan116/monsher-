...
if($this->StartResultCache(false, array($arrFilter, ($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $arNavigation, $pagerParameters)))
{
	...
	//EXECUTE
	$arrD = array_merge($arrFilter, $arFilter);
	
	
	/*
	unset($arrD["ACTIVE_DATE"]);
	unset($arrD["ACTIVE"]);
	unset($arrD["CATALOG_SHOP_QUANTITY_1"]);
	unset($arrD["MIN_PERMISSION"]);
	unset($arrD["CHECK_PERMISSIONS"]);
	unset($arrD["PROPERTY_MSK_VALUE"]);
	*/
	//pre($arrD);
	
	$rsElements = CIBlockElement::GetList($arSort, $arrD, false, $arNavParams, $arSelect);
