...
/*
				...
				{
					$arResult["TOVS"] = array();
					foreach($arResult["PROPERTIES"]["SIMILAR"]["VALUE"] as $TovID)
					{
						$mTov = CStatic::getElement($TovID);
						$arPrices = CStatic::GetPrice($TovID);
...
/*
				...
				while($arElem = $rsElem->GetNext()) { 
                    ...
                    {                        
                        if($name!='' && $href!='')
                        {    
							$arResult["PREV"]["NAME"] = $name;
							$arResult["PREV"]["HREF"] = $href;          
							$arResult["PREV"]["MODEL"] = $model;          
                        }
                        ...
                        if($arElem)
                        {  
                            $arResult["NEXT"]["NAME"] = $arElem["NAME"];
                            $arResult["NEXT"]["HREF"] = $arElem["DETAIL_PAGE_URL"];
                            $arResult["NEXT"]["MODEL"] = $arElem["PROPERTY_MODEL_VALUE"];
                        }
                    }
                    
                    $name = $arElem["NAME"];
                    $href = $arElem["DETAIL_PAGE_URL"];
                    $model = $arElem["PROPERTY_MODEL_VALUE"];
...
/*
	...
	{
		$arRes[$pid] = CIBlockFormatProperties::GetDisplayValue($arResult, $arResult["PROPERTIES"][$pid], "catalog_out");
	}
	
	$arResult["DISPLAY_PROPERTIES"] = $arRes;*/
// build DISPLAY PROPERTIES BLOCKS	
$arResult["DISPLAY_PROPERTIES_BLOCKS"] = array();
$index = 0;


$arResult["DISPLAY_PROPERTIES"] = CStatic::GetDisplayProps($arResult, $arParams["PROPERTY_CODE"]);

if($_GET["mode"] == 'test') {
	//pre($arResult["DISPLAY_PROPERTIES"]);
...
foreach($arResult["DISPLAY_PROPERTIES"] as $code => $arProp){
	if( $arProp["DEFAULT_VALUE"] == "+" ){
		$index ++;
		$arResult["DISPLAY_PROPERTIES_BLOCKS"][$index]["NAME"] = $arProp["NAME"];
		$arResult["DISPLAY_PROPERTIES_BLOCKS"][$index]["ITEMS"] = array();		
	}
	
	if( /*$arProp["FILTRABLE"] != "Y" && */$index > 0 && $arProp["VALUE"] != '+' && $arProp["VALUE"] != ''){		
		
		
		$arProp["DISPLAY_VALUE"] = preg_replace('#<a[^>]*>(.*?)</a>#is', '$1', $arProp["DISPLAY_VALUE"]);
		/*
		if( is_array($arProp["DISPLAY_VALUE"]) ){
			$arProp["DISPLAY_VALUE"] = implode('<br/>', $arProp["DISPLAY_VALUE"]);
		} else
		*/
		if( $arProp["DISPLAY_VALUE"] == 'Y' ) {
			$arProp["DISPLAY_VALUE"] = 'Есть';
		}
		$arResult["DISPLAY_PROPERTIES_BLOCKS"][$index]["ITEMS"][] = array(
			"ID" => $arProp["ID"],
			"NAME" => $arProp["NAME"],
...
if($_GET["mode"] == 'test') {
	//pre($arResult["DISPLAY_PROPERTIES_BLOCKS"]);
	
}

$arResult["LABELS"] = CStatic::GetLabelsInfo($arResult);
$arResult["LABELS_DOP"] = CStatic::GetLabelsInfoDop($arResult);
$arResult["G_PROPS_ALL"] = CStatic::DescPropAll();

$arResult = CStatic::CheckSalePrice($arResult);


//pre($arResult["G_PROPS_ALL"]);


$arResult["VIDEOS"] = array();
if($arResult["PROPERTIES"]["VIDEOS"]["VALUE"]) {
	foreach($arResult["PROPERTIES"]["VIDEOS"]["VALUE"] as $id) {
		$arResult["VIDEOS"][] = CStatic::getElement($id, CStatic::$videoIdBlock);
	}
...
if($arResult["PROPERTIES"]["YM_MODEL"]["VALUE"]) {
	
$modelId = preg_replace('~^.*\s(\d+)$~', '$1', $arResult["PROPERTIES"]["YM_MODEL"]["VALUE"]);

			$arFilter = Array(				
				'IBLOCK_ID'	=>	$arResult["IBLOCK_ID"],				
				'ACTIVE'		=>	'Y',
				'PROPERTY_YM_MODEL'	=>	'%'.$modelId,
				'!ID'			=>	$arResult["ID"]
			);
		
		$arResult["MODELS"] = CStatic::GetElementList($arFilter, 5, array("SORT"=> "ASC"), true);
