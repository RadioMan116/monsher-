...
<div class="basket-checkbox basket-checkbox_payment">
	<div class="basket-checkbox__inner">
		...
		foreach($arResult["PAY_SYSTEM"] as $arPaySystem)
		{
			if (strlen(trim(str_replace("<br />", "", $arPaySystem["DESCRIPTION"]))) > 0 || intval($arPaySystem["PRICE"]) > 0)
			{
				...
				<?
								...
								$title = $arPaySystem["PSA_NAME"];
								
								$arPaySystem["PSA_NAME"] = str_replace('с -5% скидкой','<span class="basket-checkbox__color">с -5% скидкой</span>', $arPaySystem["PSA_NAME"]);
				?>
				
				
					<!-- begin basket-checkbox__item -->
									<div class="basket-checkbox__item" for="ID_PAY_SYSTEM_ID_<?=$arPaySystem["ID"]?>" onclick="BX('ID_PAY_SYSTEM_ID_<?=$arPaySystem["ID"]?>').checked=true;changePaySystem();">
										<label class="basket-checkbox__label">
											<div class="basket-checkbox__cell">
												<input type="radio"
													id="ID_PAY_SYSTEM_ID_<?=$arPaySystem["ID"]?>"
													name="PAY_SYSTEM_ID"
													value="<?=$arPaySystem["ID"]?>"
													<?if ($arPaySystem["CHECKED"]=="Y" && !($arParams["ONLY_FULL_PAY_FROM_ACCOUNT"] == "Y" && $arResult["USER_VALS"]["PAY_CURRENT_ACCOUNT"]=="Y")) echo " checked=\"checked\"";?>
													onclick="changePaySystem();" 
													class="basket-checkbox__input"
												/>	
												<div class="basket-checkbox__icon"></div>
											</div>
											<div class="basket-checkbox__cell">
												<img src="<?=$imgUrl?>" alt="<?=$title;?>" class="basket-checkbox__pic" />
											</div>
											<div class="basket-checkbox__cell">
												<div class="basket-checkbox__text">
													<strong class="basket-checkbox__title"><?=$arPaySystem["PSA_NAME"];?></strong>
													<?=$arPaySystem["DESCRIPTION"];?>
												</div>
											</div>
										</label>
									</div>
					<!-- end basket-checkbox__item -->
